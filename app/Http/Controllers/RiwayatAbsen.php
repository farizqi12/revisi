...
class RiwayatAbsen extends Controller
{
    ...
    public function export()
    {
        try {
            ...
            $query = Absensi::with(['user', 'lokasi'])->where('user_id', $userId);
            ...
            $sheet->mergeCells('A5:K5')->setCellValue('A5', 'Nama : ' . ($user->name ?? '-'));

            // ===================== HEADER =====================
            $headers = ['No', 'Tanggal', 'Tipe', 'Status Waktu', 'Status Lokasi', 'Nama Lokasi', 'Alamat Lokasi', 'Tipe Lokasi', 'Durasi', 'Catatan'];
            ...
            foreach ($absensi as $index => $absen) {
                ...
                $sheet->setCellValue("B{$row}", $absen->created_at ? $absen->created_at->format('d-m-Y H:i:s') : '-');
                $sheet->setCellValue("C{$row}", ucfirst($absen->type ?? '-'));
                $sheet->setCellValue("D{$row}", ucfirst($absen->status_waktu ?? '-'));
                $sheet->setCellValue("E{$row}", ucfirst($absen->status_lokasi ?? '-'));
                $sheet->setCellValue("F{$row}", $absen->lokasi->name ?? '-');
                $sheet->setCellValue("G{$row}", $absen->lokasi->alamat ?? '-');
                $sheet->setCellValue("H{$row}", $absen->lokasi->type ?? '-');
                $sheet->setCellValue("I{$row}", $absen->durasi ?? '-');
                $sheet->setCellValue("J{$row}", $absen->notes ?? '-');
                $row++;
            }
